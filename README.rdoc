...
* Lexes and parses several regular expression flavors
    # In progress
      # Ruby 1.8/1.9    ** INCOMPLETE
      # POSIX BRE/ERE   ** INCOMPLETE

    # Haven't started yet
      # PCRE            ** MISSING SYNTAX DEFINITION
      # PERL            ** MISSING SYNTAX DEFINITION
      # Python          ** MISSING SYNTAX DEFINITION
      # Java            ** MISSING SYNTAX DEFINITION
      # JavaScript      ** MISSING SYNTAX DEFINITION
      # .NET            ** MISSING SYNTAX DEFINITION
...
Thinking about
  ...
  * Aspect oriented transformation "driver"
...
the expression into the smallest meaningful syntax elements (lexemes),
including their type, token (id), text, and the start/end offsets within
...
==== TODO
  * Consider extracting a "baseline" scanner (BRE?) common to all flavors, and
    layer subsets of syntax into individual ragel machines, like assertions, posix
    classes and unicode properties.

  * Add a scanner (machine) for escaped strings, usable from other machines.
