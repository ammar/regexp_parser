...
* Supports ruby 1.8 and 1.9 regular expression syntax, other flavors planned.
...
=== Parser
  ...
  root.multi_line?  # => true
  root.m?           # => true

  root.insensitive? # => false
  root.i?           # => false
...
- Escaped Sequences
  ...
  - Octal: \012 ** MISSING
- Grouped Expressions
  - Atomic: (?>abc) ** NEEDS REVIEW/TESTS
  - Capturing: (abc) ** NEEDS REVIEW/TESTS
  - Comment: (?# comment) ** NEEDS REVIEW/TESTS
  - Named: (?<name>abc) (no single quotes, yet) ** NEEDS REVIEW/TESTS
  - Options: (?mi-x:abc) ** MISSING SOME OPTIONS (u, e, etc..)
  - Passive: (?:abc) ** NEEDS REVIEW/TESTS
  - Assertions
    - Lookahead: (?=abc) ** NEEDS REVIEW/TESTS
    - Negative Lookahead: (?!abc) ** NEEDS REVIEW/TESTS
    - Lookabehind: (?<=abc) ** NEEDS REVIEW/TESTS
    - Negative Lookbehind: (?<\!abc) ** NEEDS REVIEW/TESTS
- Literals: abc
- Properties: 
  - Classes: \p{{Alpha}}, \P{{Space}}, etc...
  - Derived Properties: \p{{Math}}, \P{{Changes when Uppercased}} ** MISSING
  - General Categories: \p{{Lu}}, \P{{Cs}}, etc... ** INCOMPLETE
  - Scripts: \p{{Arabic}}, \P{{Hiragana}} ** MISSING
  - Simple Properties: \p{{Ideographic}} ** MISSING
- POSIX classes: [[:word:]], [a[:alpha:]c], etc...
- Quantifiers
  - Greedy: ?, *, + {m,n}
  - Reluctant: ??, *?, +?, {n,m}?
  - Possessive: ?++, *+++, ++++, {n,m}++  ** NOT IN ONIGURUMA?
...
* Currently, regex syntax is checked by instantiating a Regexp object from the
  ...
  will be required to support other flavors.
...
* Perl Regular Expressions Reference link[http://perldoc.perl.org/perlreref.html]
